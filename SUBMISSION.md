...
To run the API locally, set the following user secret for the `api` project:

- `Jwt:Key` = `super-long-m5-secret-key-1234567890`
...
The application reads the JWT signing key from User Secrets instead of `appsettings.json`, which reduces the risk of secrets being exposed in the repository.
